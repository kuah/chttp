...
func Test3(t *testing.T) {
	...
	if result1.Platform != "a" {
		t.Errorf("Expected Platform to be 'a', got %v", result1.Platform)
	}
	...
	if result2.Platform != "a" {
		t.Errorf("Expected Platform to be 'a', got %v", result2.Platform)
	}
	...
	// Header应该覆盖param，所以期望值是"c"
	if result3.Platform != "c" {
		t.Errorf("Expected Platform to be 'c' (header overrides param), got %v", result3.Platform)
	}

	// header with different header name - should fail validation
	req4, _ := http.NewRequest("GET", "/test", nil)
	req4.Header.Set("Content-Type", "application/json")
	req4.Header.Set("provider", "c")
	result4, parserResult, err := Valid[testStruct](req4)
	if err == nil {
		t.Errorf("Expected validation error (no platform header), got nil")
	}
	if parserResult != ParserResultNotVerified {
		t.Errorf("Expected ParserResultNotVerified (no platform header), got %v", parserResult)
	}
	// 这个测试应该失败，因为没有platform header
	if result4.Platform != "" {
		t.Errorf("Expected Platform to be empty (no platform header), got %v", result4.Platform)
	}
}
...
func TestCVTagRegression(t *testing.T) {
	...
	t.Run("cv_tag_with_json_body", func(t *testing.T) {
		...
		// JSON值应该覆盖Header值（根据优先级）
		if result.MainHeader != "json_main_header" {
			t.Errorf("Expected MainHeader to be 'json_main_header' (JSON priority), got %v", result.MainHeader)
		}

		// 嵌套结构体的JSON值应该覆盖Header值
		if result.NestedStruct.NestedHeader != "json_nested_header" {
			t.Errorf("Expected NestedStruct.NestedHeader to be 'json_nested_header' (JSON priority), got %v", result.NestedStruct.NestedHeader)
		}
	})
...
// TestGETRequestWithJSONBody 测试GET请求是否能从JSON body获取参数
func TestGETRequestWithJSONBody(t *testing.T) {
	type TestStruct struct {
		Address   *string `json:"address,omitempty" param:"address"`
		Direction *string `json:"direction,omitempty" param:"direction"`
		First     *int    `json:"first,omitempty" param:"first" default:"100"`
		EndDate   *string `json:"endDate,omitempty" param:"endDate"`
	}

	t.Run("GET_request_with_JSON_body_ignored", func(t *testing.T) {
		// 创建带有JSON body的GET请求
		body := `{
			"address": "test_address_from_json",
			"direction": "test_direction_from_json",
			"first": 200,
			"endDate": "2024-01-01"
		}`

		req, _ := http.NewRequest("GET", "/test", bytes.NewBuffer([]byte(body)))
		req.Header.Set("Content-Type", "application/json")
		...
		// 验证JSON body中的值没有被解析（GET请求忽略body）
		if result.Address != nil {
			t.Errorf("Expected Address to be nil (GET ignores body), got %v", *result.Address)
		}
		if result.Direction != nil {
			t.Errorf("Expected Direction to be nil (GET ignores body), got %v", *result.Direction)
		}
		if result.EndDate != nil {
			t.Errorf("Expected EndDate to be nil (GET ignores body), got %v", *result.EndDate)
		}

		// 只有默认值会生效
		if result.First == nil {
			t.Error("Expected First to use default value, got nil")
		} else if *result.First != 100 {
			t.Errorf("Expected First to be 100 (default), got %v", *result.First)
		}
	})

	t.Run("GET_request_with_URL_params_works", func(t *testing.T) {
		// 创建带有URL参数的GET请求
		req, _ := http.NewRequest("GET", "/test?address=test_address_from_param&direction=test_direction_from_param&first=200&endDate=2024-01-01", nil)
		...
		// 验证URL参数中的值被正确解析
		if result.Address == nil {
			t.Error("Expected Address to be set from URL param, got nil")
		} else if *result.Address != "test_address_from_param" {
			t.Errorf("Expected Address to be 'test_address_from_param', got %v", *result.Address)
		}

		if result.Direction == nil {
			t.Error("Expected Direction to be set from URL param, got nil")
		} else if *result.Direction != "test_direction_from_param" {
			t.Errorf("Expected Direction to be 'test_direction_from_param', got %v", *result.Direction)
		}

		if result.First == nil {
			t.Error("Expected First to be set from URL param, got nil")
		} else if *result.First != 200 {
			t.Errorf("Expected First to be 200, got %v", *result.First)
		}

		if result.EndDate == nil {
			t.Error("Expected EndDate to be set from URL param, got nil")
		} else if *result.EndDate != "2024-01-01" {
			t.Errorf("Expected EndDate to be '2024-01-01', got %v", *result.EndDate)
		}
	})

	t.Run("POST_request_with_JSON_body_works", func(t *testing.T) {
		// 创建带有JSON body的POST请求
		body := `{
			"address": "test_address_from_json",
			"direction": "test_direction_from_json",
			"first": 200,
		...
		// 验证JSON body中的值被正确解析（POST请求支持body）
		if result.Address == nil {
			t.Error("Expected Address to be set from JSON body, got nil")
		} else if *result.Address != "test_address_from_json" {
			t.Errorf("Expected Address to be 'test_address_from_json', got %v", *result.Address)
		}

		if result.Direction == nil {
			t.Error("Expected Direction to be set from JSON body, got nil")
		} else if *result.Direction != "test_direction_from_json" {
			t.Errorf("Expected Direction to be 'test_direction_from_json', got %v", *result.Direction)
		}

		if result.First == nil {
			t.Error("Expected First to be set from JSON body, got nil")
		} else if *result.First != 200 {
			t.Errorf("Expected First to be 200, got %v", *result.First)
		}

		if result.EndDate == nil {
			t.Error("Expected EndDate to be set from JSON body, got nil")
		} else if *result.EndDate != "2024-01-01" {
			t.Errorf("Expected EndDate to be '2024-01-01', got %v", *result.EndDate)
		}
	})
...
func TestTimeFieldParsing(t *testing.T) {
	type TimeStruct struct {
		Date1 time.Time  `json:"date1" param:"date1"`
		Date2 *time.Time `json:"date2" param:"date2"`
	}

	testCases := []struct {
		name  string
		input string
		want  string // 期望的时间字符串（统一用 2006-01-02 15:04:05 格式）
		field string // 测试哪个字段
	}{
		{"RFC3339", `{"date1":"2024-06-01T12:34:56Z"}`, "2024-06-01 12:34:56", "Date1"},
		{"Date Only", `{"date1":"2024-06-01"}`, "2024-06-01 00:00:00", "Date1"},
		{"DateTime", `{"date1":"2024-06-01 08:00:00"}`, "2024-06-01 08:00:00", "Date1"},
		{"Slash Date", `{"date1":"2024/06/01 09:10:11"}`, "2024-06-01 09:10:11", "Date1"},
		{"Dot Date", `{"date1":"2024.06.01 10:11:12"}`, "2024-06-01 10:11:12", "Date1"},
		{"TimestampSec", `{"date1":"1717171200"}`, "2024-06-01 00:00:00", "Date1"},
		{"TimestampMilli", `{"date1":"1717171200000"}`, "2024-06-01 00:00:00", "Date1"},
		{"Ptr_RFC3339", `{"date2":"2024-06-01T12:34:56Z"}`, "2024-06-01 12:34:56", "Date2"},
		{"Ptr_DateTime", `{"date2":"2024-06-01 08:00:00"}`, "2024-06-01 08:00:00", "Date2"},
		{"Ptr_TimestampSec", `{"date2":"1717171200"}`, "2024-06-01 00:00:00", "Date2"},
	}

	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			req, _ := http.NewRequest("POST", "/test", bytes.NewBuffer([]byte(tc.input)))
			req.Header.Set("Content-Type", "application/json")
			result, _, err := Valid[TimeStruct](req)
			if err != nil {
			...
			case "Date2":
				if result.Date2 == nil {
					t.Fatalf("expected Date2 to be set, got nil")
				}
				got = *result.Date2
			}
			if got.Format("2006-01-02 15:04:05") != tc.want {
				t.Errorf("%s: expected %s, got %s", tc.field, tc.want, got.Format("2006-01-02 15:04:05"))
			}
		})
	}
}

// TestPriorityIssue 测试优先级问题
func TestPriorityIssue(t *testing.T) {
	type TestStruct struct {
		MainHeader string `header:"x-main-header" json:"main_header"`
	}

	// 测试JSON和Header的优先级
	body := `{"main_header": "json_value"}`
	req, _ := http.NewRequest("POST", "/test", bytes.NewBuffer([]byte(body)))
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("X-Main-Header", "header_value")
	...
	// 根据当前逻辑，JSON应该覆盖Header
	t.Logf("Result: MainHeader=%q", result.MainHeader)
}

// TestWhatsAppTemplateFetchRequest 测试用户提供的WhatsAppTemplateFetchRequest结构体
func TestWhatsAppTemplateFetchRequest(t *testing.T) {
	type CtintR struct {
		Tenant            *string `header:"tenant,omitempty" json:"tenant,omitempty" v:"required"`
		TraceId           *string `header:"traceId,omitempty" json:"traceId,omitempty" v:"required"`
		Platform          *string `header:"platform,omitempty" json:"platform,omitempty"`
		SourceId          *string `header:"sourceId,omitempty" json:"sourceId,omitempty"`
		PreviousId        *string `header:"previousId,omitempty" json:"previousId,omitempty"`
		ReqId             *string `header:"reqId,omitempty" json:"reqId,omitempty"`
		CdssAuthorization *string `header:"cdss-authorization,omitempty" json:"cdss-authorization,omitempty"`
		GCAuthorization   *string `header:"gc_authorization,omitempty" json:"GCAuthorization,omitempty"`
	}

	type WhatsAppTemplateQueryParam struct {
		QueryParam1 string `param:"query1"`
		QueryParam2 string `param:"query2"`
	}

	type WhatsAppTemplateFetchRequest struct {
		CtintR `cv:"true"`
		// destination
		Platform                   string `json:"from" header:"platform" v:"required"`
		PlatformAccount            string `json:"platformAccount" header:"platformAccount" v:"required"`
		WhatsAppTemplateQueryParam `cv:"true"`
	}
	...
	tests := []struct {
		...
		jsonBody    string
		queryParams map[string]string
		expected    WhatsAppTemplateFetchRequest
		shouldPass  bool
		description string
	}{
		{
			name:   "platform_header_should_be_parsed",
			method: "POST",
			headers: map[string]string{
			...
			expected: WhatsAppTemplateFetchRequest{
				...
				PlatformAccount: "test_account",
				CtintR: CtintR{
					Tenant:   &[]string{"test_tenant"}[0],
					TraceId:  &[]string{"test_trace"}[0],
					Platform: &[]string{"test_platform"}[0],
				},
			},
			shouldPass:  true,
			description: "Platform header should be parsed correctly",
		},
		{
			name:   "platform_header_without_json_body",
			method: "POST",
			headers: map[string]string{
				"platform":        "test_platform",
				"platformAccount": "test_account",
			...
			shouldPass:  true,
			description: "Platform header should be parsed correctly without JSON body",
		},
		{
			name:   "platform_header_case_sensitivity",
			method: "POST",
			headers: map[string]string{
				"Platform":        "test_platform_upper", // 注意这里是大写P
				"platformAccount": "test_account",
				"tenant":          "test_tenant",
				"traceId":         "test_trace",
			},
			...
			queryParams: map[string]string{},
			expected: WhatsAppTemplateFetchRequest{
				Platform:        "test_platform_upper",
				PlatformAccount: "test_account",
				CtintR: CtintR{
					Tenant:   &[]string{"test_tenant"}[0],
					TraceId:  &[]string{"test_trace"}[0],
					Platform: &[]string{"test_platform_upper"}[0],
				},
			},
			shouldPass:  true,
			description: "Platform header should work with different case",
		},
		{
			name:   "platform_from_json_overrides_header",
			method: "POST",
			headers: map[string]string{
			...
			jsonBody: `{
				"from": "json_platform",
				"platformAccount": "json_account"
			}`,
			queryParams: map[string]string{},
			expected: WhatsAppTemplateFetchRequest{
				Platform:        "json_platform", // JSON should override header
				PlatformAccount: "json_account",
				CtintR: CtintR{
					Tenant:   &[]string{"test_tenant"}[0],
					TraceId:  &[]string{"test_trace"}[0],
					Platform: &[]string{"header_platform"}[0], // CtintR.Platform should still get header value
				},
			},
			shouldPass:  true,
			description: "JSON should override header for main Platform field",
		},
		{
			name:   "missing_required_platform_header",
			method: "POST",
			headers: map[string]string{
				"platformAccount": "test_account",
				"tenant":          "test_tenant",
			...
			expected:    WhatsAppTemplateFetchRequest{},
			shouldPass:  false,
			description: "Missing required platform header should fail validation",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// 测试 ParseWithValidation
			result, validation, err := ParseWithValidation[WhatsAppTemplateFetchRequest](req)
			if err != nil {
				t.Logf("ParseWithValidation error: %v", err)
				if tt.shouldPass {
					t.Errorf("%s: Expected no error, got %v", tt.description, err)
			...
			if tt.shouldPass && isValid {
				if result.Platform != tt.expected.Platform {
					t.Errorf("%s: Expected Platform to be %q, got %q", tt.description, tt.expected.Platform, result.Platform)
				}
				if result.PlatformAccount != tt.expected.PlatformAccount {
					t.Errorf("%s: Expected PlatformAccount to be %q, got %q", tt.description, tt.expected.PlatformAccount, result.PlatformAccount)
				}

				// 检查CtintR字段
				if tt.expected.CtintR.Tenant != nil {
					if result.CtintR.Tenant == nil {
						t.Errorf("%s: Expected CtintR.Tenant to be set, got nil", tt.description)
					} else if *result.CtintR.Tenant != *tt.expected.CtintR.Tenant {
						t.Errorf("%s: Expected CtintR.Tenant to be %q, got %q", tt.description, *tt.expected.CtintR.Tenant, *result.CtintR.Tenant)
					}
				}

				if tt.expected.CtintR.TraceId != nil {
					if result.CtintR.TraceId == nil {
						t.Errorf("%s: Expected CtintR.TraceId to be set, got nil", tt.description)
					} else if *result.CtintR.TraceId != *tt.expected.CtintR.TraceId {
						t.Errorf("%s: Expected CtintR.TraceId to be %q, got %q", tt.description, *tt.expected.CtintR.TraceId, *result.CtintR.TraceId)
					}
				}

				if tt.expected.CtintR.Platform != nil {
					if result.CtintR.Platform == nil {
						t.Errorf("%s: Expected CtintR.Platform to be set, got nil", tt.description)
					} else if *result.CtintR.Platform != *tt.expected.CtintR.Platform {
						t.Errorf("%s: Expected CtintR.Platform to be %q, got %q", tt.description, *tt.expected.CtintR.Platform, *result.CtintR.Platform)
					}
				}
			}

			// 打印调试信息
			t.Logf("Result: Platform=%q, PlatformAccount=%q, CtintR.Platform=%v",
				result.Platform,
				...
				func() string {
					...
					return *result.CtintR.Platform
				}())

			// 打印headers信息
			t.Logf("Headers: %+v", req.Header)
			t.Logf("Header.Get('platform'): %q", req.Header.Get("platform"))
			t.Logf("Header.Get('Platform'): %q", req.Header.Get("Platform"))
		})
	}
}
